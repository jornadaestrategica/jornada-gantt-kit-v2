...
export function FeatureRichGantt<TContext>(props: FeatureRichGanttProps<TContext>) {
  ...
  const onActionComplete = useEvent((value: unknown) => {
    ...
    // Salvar é tratado diferente — sempre aplica os registros modificados normalmente
    if (requestType === "save") {
    ...
    // Atualização de registro (célula ou barra) — em modo automático,
    // aguarda o cálculo nativo das sucessoras antes de sincronizar
    if (requestType === "recordupdate") {
      const modified = [...(event.modifiedRecords ?? []), event.data].map(readNativeRow)
        .filter((row): row is SyncfusionTask => row !== null)
      if (!modified.length) return
      
      // Se NÃO está em modo automático, aplica só o que foi modificado
      if (viewRef.current.scheduleMode !== "auto" || !editing) {
        run((current) => applyNativeEdits(current, modified, identities.current, {
          allowDependencies: permissions.link && capabilities.dependencies,
          allowSchedulingMode: viewRef.current.scheduleMode === "custom",
          editOptions: props.adapter.editOptions?.bind(props.adapter),
      ...
      // Em modo automático: aguardar que o Syncfusion conclua o recálculo
      // de sucessoras (updatedRecords) antes de sincronizar tudo
      requestAnimationFrame(() => {
        if (busy.current || binding.current) return
        const bySourceId = new Set(modified.map((row) => row._sourceId))
        const allUpdated = gantt.current?.updatedRecords?.map(readNativeRow)
          .filter((row): row is SyncfusionTask => row !== null && !bySourceId.has(row._sourceId) && !row._projection) ?? []
        const toApply = [...modified, ...allUpdated]
        run((current) => applyNativeEdits(current, toApply, identities.current, {
          allowDependencies: permissions.link && capabilities.dependencies,
          allowSchedulingMode: viewRef.current.scheduleMode === "custom",
          editOptions: props.adapter.editOptions?.bind(props.adapter),
        }))
  ...
  const onTaskbarInfo = useEvent((value: unknown) => {
    ...
    const row = readNativeRow(event.data)
    if (!row) return
    const color = isLateTask(row) ? "#d1483c" : row.color
    if (!color) return
    event.taskbarBgColor = color
    event.taskbarBorderColor = color
    event.progressBarBgColor = isLateTask(row) ? "#8f251d" : color
