...
/**
 * Presentation-only initial order: siblings by start date, then end date. Nothing here
 * touches the session, `task.order`, the changeset or undo/redo — it only decides the
 * order rows are first handed to the renderer, so the user can still sort manually
 * afterwards through the grid's own column headers.
 *
 * A milestone mirror is never sorted on its own date: it has no identity of its own
 * (`_sourceId` points at its owner), so it is kept immediately after the row it
 * mirrors, wherever that row lands.
 */
export function sortRowsByDate(rows: SyncfusionTask[]): SyncfusionTask[] {
  const byParent = new Map<string | null, SyncfusionTask[]>()
  const mirrorByOwner = new Map<string, SyncfusionTask>()
  for (const row of rows) {
    if (row._projection) { mirrorByOwner.set(row._sourceId, row); continue }
    const bucket = byParent.get(row.ParentID)
    if (bucket) bucket.push(row)
    else byParent.set(row.ParentID, [row])
  }
  const time = (date: Date | null) => date instanceof Date ? date.getTime() : Number.MAX_SAFE_INTEGER
  for (const bucket of byParent.values()) {
    bucket.sort((a, b) => time(a.StartDate) - time(b.StartDate) || time(a.EndDate) - time(b.EndDate) || a.order - b.order)
  }
  const result: SyncfusionTask[] = []
  const visit = (parentId: string | null): void => {
    for (const row of byParent.get(parentId) ?? []) {
      result.push(row)
      const mirror = mirrorByOwner.get(row._sourceId)
